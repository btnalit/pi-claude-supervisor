...
test("the no-output timer restarts when the Supervisor sends a turn", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-claude-supervisor-send-baseline-"));
  try {
    await initializeGitRepository(cwd, "worker/send-baseline");
    const fixture = closeOutFixture(cwd);
    // The Worker last spoke long ago (as after a long acceptance/Review run).
    fixture.handle.startedAt = new Date(Date.now() - 60_000).toISOString();
    const events = new FlakyEventLog("never-fail");
    const supervisor = new Supervisor(fixture.adapter, events as unknown as ConstructorParameters<typeof Supervisor>[1], { reviewer: automaticReviewer() });
    await supervisor.start({
      task: "a repair turn after a long verification",
      cwd,
      command: "claude",
      automation: true,
      spec: automaticSpec(),
    ...
    fixture.state.listener?.(turn);
    await supervisor.poll();
    await fixture.state.onAction?.({ action: "continue", message: "keep going", reason: "more to do" }, turn);
    fixture.state.activeRequests = 1;
    assert.equal(fixture.state.sent.length, 1);
    // Old baseline: 60s of silence → stopped on the first tick. New baseline:
    // the clock starts at the send.
    await new Promise((resolve) => setTimeout(resolve, 1_500));
    assert.equal(supervisor.state, "running");
    assert.ok(!events.events.some((event) => event.type === "worker_watchdog_timeout"));
    // A Worker that then stays silent mid-turn is still stopped.
    await waitFor(() => events.events.some((event) => event.type === "worker_watchdog_timeout"));
...
test("an idle Worker that outlasts the no-output timeout is verified, not killed", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-claude-supervisor-idle-timeout-"));
  try {
    await initializeGitRepository(cwd, "worker/idle-timeout");
    const fixture = closeOutFixture(cwd);
    const events = new FlakyEventLog("never-fail");
    const candidates: string[] = [];
    const supervisor = new Supervisor(fixture.adapter, events as unknown as ConstructorParameters<typeof Supervisor>[1], {
    ...
    await supervisor.start({
      task: "a Worker waiting on background work that never returns",
      cwd,
      command: "claude",
      automation: true,
      spec: automaticSpec(),
    ...
    fixture.state.listener?.(turn);
    await supervisor.poll();
    await fixture.state.onAction?.({ action: "wait", reason: "background agents are running" }, turn);
    await waitFor(() => supervisor.state === "completed");
    const types = events.events.map((event) => event.type);
...
test("a spent turn budget verifies the work instead of parking it as a Decision Worker failure", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-claude-supervisor-turn-budget-"));
  try {
    await initializeGitRepository(cwd, "worker/turn-budget");
    const fixture = closeOutFixture(cwd);
    const events = new FlakyEventLog("never-fail");
    const supervisor = new Supervisor(fixture.adapter, events as unknown as ConstructorParameters<typeof Supervisor>[1], { reviewer: automaticReviewer() });
    await supervisor.start({
      task: "use up the turn budget",
      cwd,
      command: "claude",
      automation: true,
      spec: automaticSpec(),
    ...
    const turn = fixture.turn(2);
    fixture.state.listener?.(turn);
    await supervisor.poll();
    await fixture.state.onAction?.({ action: "continue", message: "one more thing", reason: "not finished" }, turn);
    assert.equal(fixture.state.sent.length, 0);
    assert.equal(supervisor.state, "completed");
    assert.equal(supervisor.candidateParked, false);
    const override = events.events.find((event) => event.type === "decision_overridden");
    assert.match(String(override?.data?.reason), /turn budget of 1 is exhausted/u);
  } finally {
    await rm(cwd, { recursive: true, force: true });
  }
});

test("a verify decision that lands after the Worker resumed on its own is dropped", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-claude-supervisor-stale-verify-"));
  try {
    await initializeGitRepository(cwd, "worker/stale-verify");
    const fixture = closeOutFixture(cwd);
    const events = new FlakyEventLog("never-fail");
    const supervisor = new Supervisor(fixture.adapter, events as unknown as ConstructorParameters<typeof Supervisor>[1], { reviewer: automaticReviewer() });
    await supervisor.start({
      task: "background work re-invokes the Worker",
      cwd,
      command: "claude",
      automation: true,
      spec: automaticSpec(),
    ...
    await supervisor.poll();
    fixture.state.activeRequests = 1;
    await fixture.state.onAction?.({ action: "verify", reason: "looks done" }, turn);
    assert.equal(supervisor.state, "waiting");
    assert.equal(fixture.state.stopReasons.length, 0);
    assert.ok(events.events.some((event) => event.type === "decision_ignored"));
  } finally {
    await rm(cwd, { recursive: true, force: true });
  }
...
test("a Worker under human takeover is not stopped for silence", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-claude-supervisor-takeover-silence-"));
  try {
    await initializeGitRepository(cwd, "worker/takeover-silence");
    const fixture = closeOutFixture(cwd);
    fixture.handle.startedAt = new Date(Date.now() - 60_000).toISOString();
    const events = new FlakyEventLog("never-fail");
    const supervisor = new Supervisor(fixture.adapter, events as unknown as ConstructorParameters<typeof Supervisor>[1], { reviewer: automaticReviewer() });
    await supervisor.start({
      task: "a recovered task waiting for its operator",
      cwd,
      command: "claude",
      automation: true,
      spec: automaticSpec(),
...
/** An automatic task whose Worker send is scripted, driven by Decision actions the test injects. */
async function withScriptedSend(
  send: (message: string) => Promise<void>,
  run: (context: { supervisor: Supervisor; handle: WorkerHandle; events: FlakyEventLog; decide: Parameters<DecisionWorkerFactory>[0]["onAction"]; fail: NonNullable<Parameters<DecisionWorkerFactory>[0]["onFailure"]>; emit: (event: WorkerEvent) => void }) => Promise<void>,
  inputRetryBaseMs = 5,
): Promise<void> {
  const handle: WorkerHandle = { id: "input-retry-worker", startedAt: new Date().toISOString(), cwd: process.cwd(), ownership: "owned" };
  let running = true;
  let options: Parameters<DecisionWorkerFactory>[0] | undefined;
  let eventListener: WorkerStartInput["eventListener"] | undefined;
  const adapter: WorkerAdapter = {
    capabilities: () => ({ transport: "tmux", interactiveInput: true, pause: true, resumeSession: false, processGroupControl: true, persistentSession: true }),
    start: async (input) => { eventListener = input.eventListener; return handle; },
    getStatus: async () => ({ handle, running, activeRequests: 0, processGroupCleaned: !running }),
    readOutput: async () => [],
    send: async (_handle, message) => send(message),
    pause: async () => {},
    resume: async () => {},
    stop: async () => { running = false; },
    killProcessGroup: async () => { running = false; },
  ...
  try {
    await run({ supervisor, handle, events, decide: (action, event) => options!.onAction(action, event), fail: (event, error) => options!.onFailure!(event, error), emit: (event) => { void eventListener?.(event); } });
  } finally {
    if (!["completed", "blocked", "failed", "stopped"].includes(supervisor.state)) await supervisor.stop("test cleanup").catch(() => {});
  }
}
...
test("a decision the Worker cannot take yet is re-sent later instead of parking the task", async () => {
  const sent: string[] = [];
  let refusals = 2;
...
test("a Worker that never takes input is parked with the real reason after bounded retries", async () => {
  ...
  await withScriptedSend(async () => { attempts += 1; throw new WorkerInputError("prompt is busy", { retryable: true }); }, async ({ supervisor, handle, events, decide, emit }) => {
    ...
    const blocked = events.events.find((event) => event.type === "candidate_blocked" || event.type === "human_required");
    assert.match(JSON.stringify(blocked?.data ?? events.events.map((event) => event.data)), /did not accept input after 5 deferred attempts/u);
  });
});
...
test("fresh Worker activity supersedes a deferred decision message", async () => {
  ...
  await withScriptedSend(async () => { attempts += 1; throw new WorkerInputError("prompt is busy", { retryable: true }); }, async ({ supervisor, handle, decide, emit }) => {
    ...
    // Well past the first retry delay.
...
test("a failed Worker send is reported as an input failure, not a Decision Worker failure", async () => {
  await withScriptedSend(async () => {}, async ({ supervisor, handle, events, fail }) => {
    await fail({ type: "turn_completed", handle, result: {}, sequence: 1 }, new WorkerInputError("the message is still in Claude's input box", { retryable: false }));
    assert.equal(supervisor.candidateParked, true);
    assert.ok(events.events.some((event) => event.type === "worker_input_failed"));
...
for (const origin of ["idle watchdog", "close-out"] as const) {
  test(`a verification the ${origin} starts that throws part-way parks the candidate instead of hanging in verifying`, async () => {
    const cwd = await mkdtemp(join(tmpdir(), "pi-claude-supervisor-own-verify-throws-"));
    try {
      await initializeGitRepository(cwd, "worker/own-verify-throws");
      const fixture = closeOutFixture(cwd);
      const events = new FlakyEventLog("acceptance_started");
      const supervisor = new Supervisor(fixture.adapter, events as unknown as ConstructorParameters<typeof Supervisor>[1], { reviewer: automaticReviewer() });
      await supervisor.start({
        task: `${origin} verification throws`,
        cwd,
        command: "claude",
        automation: true,
        spec: automaticSpec(),
        ...(origin === "close-out"
          ? { startedAt: new Date(Date.now() - 3_000).toISOString(), deadlineMs: 2_000, deadlineGraceMs: 60_000, deadlineWarningMs: 0, noOutputTimeoutMs: 0 }
          : { deadlineMs: 0, noOutputTimeoutMs: 500 }),
        decisionWorkerFactory: fixture.decisionWorkerFactory,
      });
      const turn = fixture.turn(1);
      fixture.state.listener?.(turn);
      await supervisor.poll();
      // The decision for the turn is backing off an outage, so the Supervisor verifies on its own.
      fixture.state.onRetry?.(turn, { attempt: 1, delayMs: 60_000, error: new Error("529 overloaded_error") });
      await waitFor(() => supervisor.state === "blocked");
      assert.equal(supervisor.candidateParked, true);
      assert.ok(events.events.some((event) => event.type === "worker_event_error" && /injected event failure/u.test(String(event.data?.error))));
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
